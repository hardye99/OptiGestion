...
import { CitaConCliente } from '@/lib/types'; // <-- 1. IMPORTAR EL TIPO
...
      .eq('estado', 'pendiente');
    ...
    // 2. APLICAR TYPE CASTING AL RESULTADO
    const { data: citas, error } = result as unknown as { 
        data: CitaConCliente[] | null, 
        error: any 
    };
...
      detalles: resultados.map((r, i) => {
            const citaItem = citas[i] as any; // <-- CAST LOCAL PARA EVITAR ERROR DE TIPADO EN EL MAP
            return ({
              cita: citaItem.id,
              cliente: citaItem.cliente?.email,
