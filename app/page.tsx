...
import { useAuth } from "@/lib/auth-context"; // <-- AGREGADO: Importar useAuth

export default function HomePage() {
  // <-- AGREGADO: Obtener el estado de autenticación y perfil
  ...
  const cargarEstadisticas = async () => {
    // Si no hay usuario o perfil, no se ejecuta la carga de datos.
    ...
    try {
      ...
      const totalVentasMes = salidasMes?.reduce((acc, salida) => {
        const precio = (salida as any).producto?.precio || 0;
  ...
  useEffect(() => {
    // Solo cargar estadísticas si el usuario y el perfil están listos
    if (user && profile) {
      cargarEstadisticas();
    }
    // Añadir dependencias faltantes para evitar advertencia de hook
  }, [user, profile]);

  // <-- AGREGADO: Pantalla de carga mientras AuthContext obtiene los datos
  if (loading || !user || !profile) {
      return (
        <div className="flex items-center justify-center h-[80vh]">
          <p className="text-gray-500 text-xl">Preparando dashboard...</p>
        </div>
      );
  }
  // ----------------------------------------------------------------------
